...
void protocol::handle_line(SBBF& store, std::string_view line, std::string& out) {
  line = trim(line);
  if (line.empty()) return;

  auto sp = line.find(' ');
  std::string_view cmd = (sp == std::string_view::npos) ? line : line.substr(0, sp);
  std::string_view val = (sp == std::string_view::npos) ? std::string_view{} : trim(line.substr(sp + 1));
